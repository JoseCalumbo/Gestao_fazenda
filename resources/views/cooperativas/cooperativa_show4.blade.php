          ...
          <div class="settings-panel" id="tab-talhoes">
            <div class="cfg-card anim">
              ...
              <div class="filter-bar" id="talhoesFiltros">
                ...
                <select class="filter-select" id="filtroTalhaoAgricultor">
                  <option value="">Todos os agricultores</option>
                </select>
                <button class="btn-green btn-filter" id="btnFiltrarTalhoes" style="padding:8px 18px;"><i
                    class="bi bi-search"></i> Filtrar</button>
                <button class="btn-outline-green btn-filter" id="btnLimparFiltrosTalhoes"
                  style="padding:8px 18px;"><i class="bi bi-eraser"></i> Limpar</button>
              </div>
        ...
        <div class="modal-header">
          <div style="display:flex;align-items:center;gap:14px;flex:1;">
            ...
            <div>
              ...
              <div style="font-size:12px;color:rgba(255,255,255,.65);margin-top:2px;" id="modalTalhaoSub">Nova parcela de
        ...
        <div class="modal-header">
          <div style="display:flex;align-items:center;gap:14px;flex:1;">
            ...
            <div>
              ...
              <div style="font-size:12px;color:rgba(255,255,255,.65);margin-top:2px;" id="modalVendaSub">Registo de venda
                de produtos</div>
            </div>
          </div>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
  ...
  <div class="modal fade" id="modalDeleteVenda" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px;">
      <div class="modal-content">
        ...
        <div class="modal-footer" style="border-top:1px solid #FFCDD2;">
          <button type="button" class="btn-outline-green" data-bs-dismiss="modal">Cancelar</button>
          <button type="button" class="btn-green" id="btnConfirmDeleteVenda" style="background:#C62828;box-shadow:none;">
            <i class="bi bi-trash-fill"></i> Eliminar Definitivamente
          </button>
        </div>
      </div>
...
══════════════════════════════════════ -->
  <script>
    ...
    function carregarTalhoes(page = 1) {
      ...
      fetch(`/api/cooperativa/${cooperativaId}/talhoes?${params}`)
        .then(res => res.json())
        .then(data => {
    ...
    document.getElementById('btnFiltrarTalhoes').addEventListener('click', () => {
      ...
      talhoesFiltros.agricultor_id = document.getElementById('filtroTalhaoAgricultor').value;
      carregarTalhoes(1);
    });
    ...
    document.getElementById('btnLimparFiltrosTalhoes').addEventListener('click', () => {
      ...
      document.getElementById('filtroTalhaoAgricultor').value = '';
      talhoesFiltros = { designacao: '', estado: '', agricultor_id: '' };
      carregarTalhoes(1);
    });

    /* Popular filtro de agricultores */
        ...
        .then(data => {
          ...
          fetch(`/api/cooperativa/${cooperativaId}/agricultores/associados`)
            ...
            .then(agData => {
              ...
              select.innerHTML = '<option value="">Selecione um agricultor</option>' +
                agData.data.map(a =>
                  `<option value="${a.id}" ${a.id == t.agricultor_id ? 'selected' : ''}>${a.nome}</option>`
                ).join('');
              modalTalhao.show();
            });
        })
    ...
    document.getElementById('btnSalvarTalhao').addEventListener('click', () => {
      ...
      const url = id ? `/api/cooperativa/${cooperativaId}/talhoes/${id}` :
        `/api/cooperativa/${cooperativaId}/talhoes`;
    ...
    function carregarInsumos(page = 1) {
      ...
      fetch(`/api/cooperativa/${cooperativaId}/insumos?${params}`)
        .then(res => res.json())
        .then(data => {
