...
<head>
    ...
    <title>SIAG – Vendas - {{ $cooperativa->nome ?? 'Cooperativa' }}</title>
    ...
    <style>
        ...
        *::after {
        ...
        .modal-col-dados .dados-scroll::-webkit-scrollbar {
            width: 4px;
        }

        .modal-col-dados .dados-scroll::-webkit-scrollbar-thumb {
            background: rgba(0, 0, 0, .12);
            border-radius: 4px;
        }
        ...
        .troco-info .troco-valor.negativo {
            color: var(--danger);
        }
        ...
        .resumo-itens .resumo-body::-webkit-scrollbar {
            width: 4px;
        }

        .resumo-itens .resumo-body::-webkit-scrollbar-thumb {
            background: rgba(0, 0, 0, .12);
            border-radius: 4px;
        }
        ...
        .resumo-itens .resumo-item:last-child {
            border-bottom: none;
        }

        .resumo-itens .resumo-item .item-nome {
            font-weight: 500;
            color: var(--text-dark);
            flex: 1;
        }

        .resumo-itens .resumo-item .item-qtd {
            margin: 0 12px;
            color: var(--text-mid);
            font-weight: 500;
        }

        .resumo-itens .resumo-item .item-subtotal {
            font-weight: 600;
            color: var(--primary);
            min-width: 80px;
            text-align: right;
        ...
        .resumo-total-modal .total-valor {
            color: var(--primary);
        }
        ...
        .modal-resumo .modal-header .btn-close {
        ...
        .modal-resumo .modal-body::-webkit-scrollbar {
            width: 6px;
        }
        ...
        .fatura-preview .fatura-cliente .cliente-nome {
            font-weight: 600;
            font-size: 15px;
        }

        .fatura-preview .fatura-cliente .cliente-detalhe {
            font-size: 12px;
            color: var(--text-light);
        }
        ...
        .fatura-preview .fatura-itens table td:last-child {
            text-align: right;
        }
        ...
        .fatura-preview .fatura-total .total-valor {
            color: var(--primary);
            font-size: 20px;
        }
        ...
        body.dark-mode .modal-col-dados {
            background: #1e2a20;
        }

        body.dark-mode .modal-col-resumo {
            background: #141d15;
        }
        ...
        body.dark-mode .fatura-preview {
            background: #1e2a20;
            border-color: rgba(255, 255, 255, .07);
        }

        body.dark-mode .modal-resumo .modal-body {
            background: #1a2a1c;
        }

        body.dark-mode .resumo-itens .resumo-header {
            background: #172518;
        }

        body.dark-mode .troco-info {
            background: rgba(102, 187, 106, .15);
        }

        body.dark-mode .modal-resumo .modal-body::-webkit-scrollbar-thumb {
...
<body>
    ...
    <header id="topbar">
        ...
        <span class="topbar-title">Vendas - {{ $cooperativa->nome ?? 'Cooperativa' }}</span>
        <nav aria-label="breadcrumb" class="d-none d-md-flex ms-3">
            <ol class="breadcrumb mb-0" style="font-size:12.5px;">
                ...
                <li class="breadcrumb-item"><a href="{{ route('vendas') }}"
        ...
        <div class="topbar-right">
            <span class="badge rounded-pill d-none d-md-inline-flex align-items-center gap-1"
                ...
                <i class="bi bi-building"></i> {{ $cooperativa->nome ?? 'Cooperativa' }}
            </span>
    ...
    <main id="main">
        <div class="content-inner">
            ...
            <div class="page-header anim">
                <div>
                    <h1>{{ $cooperativa->nome ?? 'Cooperativa' }}</h1>
                    <p>Gestão de vendas de produtos agrícolas</p>
                </div>
            ...
            <div class="filter-btn-group anim anim-d1" id="filterButtons">
                ...
                <button class="filter-btn" data-filter="horticolas">
                    Hortícolas <span class="badge-count" id="countHorticolas">0</span>
                </button>
                <button class="filter-btn" data-filter="verduras">
                    Verduras <span class="badge-count" id="countVerduras">0</span>
                </button>
            </div>
                    ...
                    <div class="table-card">
                        ...
                        <div class="table-card-header">
                            <div style="display:flex;align-items:center;gap:12px;">
                                <h5><i class="bi bi-box-seam-fill me-2" style="color:var(--primary);"></i>Produtos
                                    Disponíveis</h5>
                            </div>
                            <div style="display:flex;gap:8px;align-items:center;">
                                <span style="font-size:12.5px;color:var(--text-light);" id="produtoCount">0
                                    produtos</span>
                            </div>
                        </div>
                        ...
                        <div class="search-filter-bar">
                            ...
                            <button class="btn-green" id="btnFiltrar" style="padding:8px 18px;"><i
            ...
            <div class="table-card anim anim-d3 mt-4">
                <div class="table-card-header">
                    <div style="display:flex;align-items:center;gap:12px;">
                        <h5><i class="bi bi-clock-history me-2" style="color:var(--primary);"></i>Histórico de Vendas
                        </h5>
                    </div>
                    <div style="display:flex;gap:8px;align-items:center;">
                        <span style="font-size:12.5px;color:var(--text-light);" id="vendaTotalCount">0 registos</span>
                        <button class="btn-outline-green btn-sm" id="btnRefreshVendas" title="Atualizar">
                ...
                <div class="search-filter-bar">
                    <div class="search-wrap" style="flex:1;min-width:150px;">
                        <i class="bi bi-search"></i>
                        <input type="text" class="search-input" id="searchCliente"
                            placeholder="Pesquisar por cliente..." style="padding-left:36px;">
                    </div>
                    <select class="filter-select" id="filterPagamentoVenda">
                    ...
                    <input type="date" class="filter-select" id="filterDataInicio" style="min-width:130px;"
                        placeholder="Data início">
                    <input type="date" class="filter-select" id="filterDataFim" style="min-width:130px;"
                        placeholder="Data fim">
                    <button class="btn-green" id="btnFiltrarVendas" style="padding:8px 18px;">
                        <i class="bi bi-search"></i> Filtrar
                    </button>
                    <button class="btn-outline-green" id="btnLimparFiltrosVendas" style="padding:8px 18px;">
                ...
                <div style="overflow-x:auto;">
                    <table class="venda-table" id="vendaTable">
                        <thead>
                            <tr>
                                <th style="cursor:pointer;" onclick="sortVendas('numero')">
                                    Nº Venda <i class="bi bi-arrow-down-up"></i>
                                </th>
                                <th style="cursor:pointer;" onclick="sortVendas('data')">
                                    Data <i class="bi bi-arrow-down-up"></i>
                                </th>
                                <th style="cursor:pointer;" onclick="sortVendas('cliente')">
                                    Cliente <i class="bi bi-arrow-down-up"></i>
                                </th>
                                <th>Itens</th>
                                <th style="cursor:pointer;" onclick="sortVendas('total_raw')">
                                    Total (Kz) <i class="bi bi-arrow-down-up"></i>
                                </th>
                                <th>Pagamento</th>
                ...
                <!-- Paginação -->
                <div class="table-footer">
                    <span id="vendaInfo">Mostrando 0 de 0 vendas</span>
                    <div class="pagination-btns" id="vendaPagination">
                        <button class="page-btn" id="vendaPrevPage" disabled>
                            <i class="bi bi-chevron-left"></i>
                        </button>
                        <span class="page-btn active" id="vendaCurrentPage">1</span>
                        <button class="page-btn" id="vendaNextPage" disabled>
                            <i class="bi bi-chevron-right"></i>
                        </button>
                        <select class="filter-select" id="vendaPerPage"
                            style="min-width:70px;padding:4px 24px 4px 10px;font-size:12px;height:34px;">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <div style="display:flex;align-items:center;gap:14px;flex:1;">
                        ...
                        <div>
                            <div class="modal-title">Finalizar Venda - {{ $cooperativa->nome ?? 'Cooperativa' }}</div>
                            <div style="font-size:12px;color:rgba(255,255,255,.65);margin-top:2px;">Preencha os dados e
                                confirme</div>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-col-resumo">
                        ...
                        <div class="resumo-itens">
                            ...
                            <div class="resumo-body" id="resumoItensLista"></div>
                        </div>
                    ...
                    <div class="modal-col-dados">
                        <div class="dados-scroll">
                            ...
                            <form id="formFinalizarVenda">
                                @csrf
                                <input type="hidden" id="cooperativaId" value="{{ $cooperativa->id ?? 1 }}">
                                <input type="hidden" id="cooperativaNome"
                                    value="{{ $cooperativa->nome ?? 'Cooperativa' }}">
                                <div class="row g-3">
                                    <div class="col-12">
                                        <label class="cfg-label" for="clienteNome">Nome do Cliente *</label>
                                        <input class="cfg-input" type="text" id="clienteNome" required
                                    ...
                                    <div class="col-12 col-md-6">
                                        ...
                                        <input class="cfg-input" type="number" id="valorEntregue" placeholder="0"
                                            min="0" step="1">
                                    </div>
                                    <div class="col-12">
    ...
    <div class="modal fade modal-resumo" id="modalResumoVenda" tabindex="-1" aria-hidden="true"
        data-bs-backdrop="static" data-bs-keyboard="false">
                ...
                <div class="modal-header">
                    <div style="display:flex;align-items:center;gap:14px;flex:1;">
                        ...
                        <div>
                            ...
                            <div style="font-size:12px;color:rgba(255,255,255,.65);margin-top:2px;" id="resumoNumero">
                                #000000</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-body">
                    <div class="fatura-preview" id="faturaPreview">
                        <div class="fatura-header">
                            <div>
                                <div class="fatura-titulo" id="faturaCooperativaNome">{{ $cooperativa->nome ?? 'SIAG' }}
                                </div>
                                <div style="font-size:12px;color:var(--text-light);">Sistema Integrado de Apoio à Gestão
                                    Agrícola</div>
                            </div>
                            <div>
                        ...
                        <div class="fatura-itens">
                            <table>
                                ...
                                <tbody id="resumoItensBody"></tbody>
                            </table>
                        </div>
    ...
    <div class="modal fade modal-detalhes" id="modalDetalhesVenda" tabindex="-1" aria-hidden="true"
            ...
            <div class="modal-content">
                    ...
                    <div style="display:flex;align-items:center;gap:14px;flex:1;">
                        ...
                        <div>
                            ...
                            <div style="font-size:12px;color:rgba(255,255,255,.65);margin-top:2px;"
                                id="detalhesVendaNumero">#000000</div>
                        </div>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <div
                                style="font-size:11px;color:var(--text-light);text-transform:uppercase;font-weight:600;">
                                Data</div>
                            <div style="font-size:14px;font-weight:600;" id="detalhesData">—</div>
                        </div>
                        <div class="col-6">
                            <div
                                style="font-size:11px;color:var(--text-light);text-transform:uppercase;font-weight:600;">
                                Cliente</div>
                            <div style="font-size:14px;font-weight:600;" id="detalhesCliente">—</div>
                        </div>
                        <div class="col-6">
                            <div
                                style="font-size:11px;color:var(--text-light);text-transform:uppercase;font-weight:600;">
                                Forma de Pagamento</div>
                            <div style="font-size:14px;font-weight:600;" id="detalhesPagamento">—</div>
                        </div>
                        <div class="col-6">
                            <div
                                style="font-size:11px;color:var(--text-light);text-transform:uppercase;font-weight:600;">
                                Total</div>
                            <div style="font-size:18px;font-weight:700;color:var(--primary);" id="detalhesTotal">0 Kz
                            </div>
                        </div>
                        <div class="col-6">
                            <div
                                style="font-size:11px;color:var(--text-light);text-transform:uppercase;font-weight:600;">
                                Valor Entregue</div>
                            <div style="font-size:14px;font-weight:600;" id="detalhesEntregue">0 Kz</div>
                        </div>
                        <div class="col-6">
                            <div
                                style="font-size:11px;color:var(--text-light);text-transform:uppercase;font-weight:600;">
                                Troco</div>
                            <div style="font-size:14px;font-weight:600;" id="detalhesTroco">0 Kz</div>
                        </div>
                    </div>
                    ...
                    <!-- Itens da Venda -->
                    <div style="font-size:13px;font-weight:600;color:var(--text-dark);margin-bottom:12px;">
                        <i class="bi bi-box-seam-fill me-2" style="color:var(--primary);"></i> Itens da Venda
                    </div>
                    <div style="max-height:300px;overflow-y:auto;border:1px solid var(--border);border-radius:10px;">
                        <table class="table table-sm mb-0" style="font-size:13px;">
                            <thead style="background:#FAFBFA;position:sticky;top:0;z-index:1;">
                                <tr>
                                    <th style="padding:10px 16px;">Produto</th>
                                    <th style="padding:10px 16px;text-align:center;width:80px;">Qtd</th>
                                    <th style="padding:10px 16px;text-align:right;width:120px;">Preço</th>
                                    <th style="padding:10px 16px;text-align:right;width:120px;">Subtotal</th>
                                </tr>
                            </thead>
                    ...
                    <!-- Observações -->
                    <div id="detalhesObservacoes"
                        style="margin-top:12px;padding:12px 16px;background:#FAFBFA;border-radius:8px;display:none;">
                        <div
                            style="font-size:11px;color:var(--text-light);text-transform:uppercase;font-weight:600;margin-bottom:4px;">
                            Observações</div>
                        <div style="font-size:13px;color:var(--text-mid);" id="detalhesObsTexto">—</div>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn-green" id="btnImprimirDetalhes">
                        <i class="bi bi-printer-fill"></i> Imprimir Fatura
                    </button>
                </div>
            </div>
    ...
    <script>
        /* =============================================
           CONFIGURAÇÕES
        ============================================= */
        const COOPERATIVA_ID = {{ $cooperativa->id ?? 1 }};
        const COOPERATIVA_NOME = '{{ $cooperativa->nome ?? "Cooperativa" }}';
        const PRODUTOS_URL = `/cooperativas/${COOPERATIVA_ID}/vendas/produtos`;
        const STORE_VENDA_URL = `{{ route('vendas.store', ['cooperativaId' => $cooperativa->id ?? 1]) }}`;

        const VENDAS_LIST_URL = `/cooperativas/${COOPERATIVA_ID}/vendas/list`;
        ...
        function updateFilterCounts() {
            ...
            const frutas = produtos.filter(p => p.categoria === 'Frutas').length;
            const graos = produtos.filter(p => p.categoria === 'Grãos').length;
            const legumes = produtos.filter(p => p.categoria === 'Legumes').length;
            const tuberculos = produtos.filter(p => p.categoria === 'Tuberculos').length;
            const verduras = produtos.filter(p => p.categoria === 'Verduras').length;
            const horticolas = produtos.filter(p => p.categoria === 'Hortícolas').length;
            ...
            document.getElementById('countHorticolas').textContent = horticolas;
            document.getElementById('countVerduras').textContent = verduras;
        }
        ...
        document.getElementById('searchProduto').addEventListener('keyup', (e) => {
            if (e.key === 'Enter') { currentPage = 1; renderProducts(); }
        });
        ...
        document.getElementById('btnConfirmarVenda').addEventListener('click', async function () {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    document.getElementById('faturaCooperativaNome').textContent = COOPERATIVA_NOME;

                    // Salvar ID da venda no dataset para impressão
                    document.getElementById('resumoNumero').dataset.vendaId = result.venda_id;
                    ...
                    // Recarregar histórico de vendas
                    loadVendas();
        ...
        ============================================= */
        function imprimirFatura(venda) {
            const data = new Date(venda.data_original || Date.now());
            ...
            const itensHtml = venda.itens.map(item => `
                <tr>
                    <td style="padding:6px 0;border-bottom:1px solid #eee;">${item.produto_nome}</td>
                    <td style="padding:6px 0;border-bottom:1px solid #eee;text-align:center;">${item.quantidade}</td>
                    <td style="padding:6px 0;border-bottom:1px solid #eee;text-align:right;">${Number(item.preco_unitario).toLocaleString('pt-AO')} Kz</td>
                    <td style="padding:6px 0;border-bottom:1px solid #eee;text-align:right;">${Number(item.subtotal).toLocaleString('pt-AO')} Kz</td>
                </tr>
            `).join('');

            const total = Number(venda.valor_total).toLocaleString('pt-AO');
            const entregue = Number(venda.valor_entregue || 0).toLocaleString('pt-AO');
            const troco = Number(venda.troco || 0).toLocaleString('pt-AO');

            const html = `
                <!DOCTYPE html>
                <html>
                <head>
                    <title>Fatura - ${venda.cooperativa_nome || COOPERATIVA_NOME}</title>
                    <meta charset="UTF-8">
                    <style>
                        * { margin: 0; padding: 0; box-sizing: border-box; }
                        ...
                        .fatura-itens th:last-child,
                        .fatura-itens td:last-child {
                            text-align: right;
                        }
                        .fatura-itens td {
                            padding: 8px 0;
                        ...
                        .fatura-total .total-row .total-valor {
                            color: #2E7D32;
                            font-size: 22px;
                        }
                        .fatura-troco {
                        ...
                        @media print {
                            body { padding: 20px; }
                            .fatura-container { border: none; box-shadow: none; padding: 20px; }
                        }
                    </style>
                </head>
                <body>
                    <div class="fatura-container">
                        <div class="fatura-header">
                            <div>
                                <div class="fatura-titulo">${venda.cooperativa_nome || COOPERATIVA_NOME}</div>
                                <div class="fatura-subtitulo">Sistema Integrado de Apoio à Gestão Agrícola</div>
                            </div>
                            <div>
                                <div class="fatura-numero">
                                    <strong>#${venda.numero}</strong><br>
                                    <span style="font-size:12px;">${dataFormatada}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="fatura-total">
                            <div class="total-row">
                                <span>Total</span>
                                <span class="total-valor">${total} Kz</span>
                            </div>
                        </div>

                        <div class="fatura-troco">
                            <span>Valor Entregue: ${entregue} Kz</span>
                            <span>Troco: ${troco} Kz</span>
                        </div>

                        ${venda.observacoes ? `
                            <div class="fatura-observacoes">
                        ...
                        <div class="fatura-rodape">
                            Documento gerado automaticamente · Válido sem assinatura<br>
                            SIAG - Sistema Integrado de Apoio à Gestão Agrícola
                        </div>
                    </div>
        ...
        document.getElementById('btnImprimirResumo').addEventListener('click', function () {
            const vendaId = document.getElementById('resumoNumero').dataset.vendaId;
            if (vendaId) {
                carregarEImprimirVenda(parseInt(vendaId));
            } else {
                showToast('Erro', 'Não foi possível identificar a venda.', 'danger');
            }
        });

        document.getElementById('btnImprimirDetalhes').addEventListener('click', function () {
            if (vendaAtualDetalhes) {
                imprimirFatura(vendaAtualDetalhes);
            } else {
                showToast('Erro', 'Não foi possível identificar a venda.', 'danger');
            }
        });
        ...
        let vendaPerPage = 10;
        let vendaSortField = 'created_at';
        let vendaSortDirection = 'desc';
        let vendaFilters = {
            cliente: '',
            forma_pagamento: '',
            data_inicio: '',
        ...
        let vendaAtualDetalhes = null;

        function loadVendas() {
            showLoading(true);

            const params = new URLSearchParams({
                page: vendaCurrentPage,
                per_page: vendaPerPage,
                sort: vendaSortField,
            ...
            fetch(`${VENDAS_LIST_URL}?${params}`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        renderPaginacaoVendas(data.pagination);
                        updateVendaCount(data.pagination);
                    } else {
                        showToast('Erro', data.message || 'Falha ao carregar vendas.', 'danger');
                    }
                })
                .catch(err => {
        ...
        function renderTabelaVendas(vendasList) {
            ...
            tbody.innerHTML = vendasList.map(v => `
                <tr data-venda-id="${v.id}">
                    ...
                    <td>${v.itens_count} itens</td>
                    <td><strong>${v.total}</strong></td>
                    <td>
                        <span class="badge" style="font-size:11px;padding:4px 10px;border-radius:20px;background:${v.forma_pagamento === 'Dinheiro' ? '#E8F5E9' : v.forma_pagamento === 'Transferência' ? '#E3F2FD' : v.forma_pagamento === 'Crédito' ? '#FFF8E1' : '#F5F5F5'};color:${v.forma_pagamento === 'Dinheiro' ? '#2E7D32' : v.forma_pagamento === 'Transferência' ? '#1565C0' : v.forma_pagamento === 'Crédito' ? '#F57F17' : '#757575'};">
                            ${v.forma_pagamento}
                        </span>
                    </td>
        ...
        function renderPaginacaoVendas(pagination) {
            if (!pagination) return;

            const currentPage = pagination.current_page || 1;
            const lastPage = pagination.last_page || 1;

            document.getElementById('vendaCurrentPage').textContent = currentPage;
            document.getElementById('vendaPrevPage').disabled = currentPage <= 1;
            document.getElementById('vendaNextPage').disabled = currentPage >= lastPage;
        }

        function updateVendaCount(pagination) {
            if (!pagination) return;
            ...
            document.getElementById('vendaTotalCount').textContent = total + ' registos';
            document.getElementById('vendaInfo').textContent =
                `Mostrando ${from} até ${to} de ${total} vendas`;
        }

        function applyVendaFilters() {
            vendaFilters.cliente = document.getElementById('searchCliente').value.trim();
            vendaFilters.forma_pagamento = document.getElementById('filterPagamentoVenda').value;
            vendaFilters.data_inicio = document.getElementById('filterDataInicio').value;
            vendaFilters.data_fim = document.getElementById('filterDataFim').value;

            vendaCurrentPage = 1;
            loadVendas();
        }

        function clearVendaFilters() {
            document.getElementById('searchCliente').value = '';
            document.getElementById('filterPagamentoVenda').value = '';
        ...
        document.getElementById('vendaPrevPage').addEventListener('click', () => {
            if (vendaCurrentPage > 1) { vendaCurrentPage--; loadVendas(); }
        });
        ...
        document.getElementById('vendaPerPage').addEventListener('change', function () {
            vendaPerPage = parseInt(this.value);
            vendaCurrentPage = 1;
            loadVendas();
        });

        document.getElementById('btnRefreshVendas').addEventListener('click', () => {
            loadVendas();
            showToast('Atualizado', 'Lista de vendas atualizada.');
        });

        document.getElementById('btnFiltrarVendas').addEventListener('click', applyVendaFilters);
        document.getElementById('btnLimparFiltrosVendas').addEventListener('click', clearVendaFilters);

        document.getElementById('searchCliente').addEventListener('keyup', (e) => {
            if (e.key === 'Enter') { applyVendaFilters(); }
        });

        /* =============================================
           DETALHES E IMPRESSÃO DA VENDA
        ============================================= */
        const modalDetalhes = new bootstrap.Modal(document.getElementById('modalDetalhesVenda'));

        function carregarDetalhesVenda(vendaId) {
            showLoading(true);

            fetch(`/cooperativas/${COOPERATIVA_ID}/vendas/${vendaId}`)
                .then(res => {
                    if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
                    return res.json();
                })
                .then(data => {
                    if (data.success) {
                        vendaAtualDetalhes = data.venda;
                        renderizarDetalhesVenda(data.venda);
                        modalDetalhes.show();
                    } else {
                        showToast('Erro', data.message || 'Falha ao carregar detalhes da venda.', 'danger');
                    }
                })
                .catch(err => {
                    console.error('Erro ao carregar detalhes da venda:', err);
                    showToast('Erro', 'Falha ao carregar detalhes da venda.', 'danger');
                })
                .finally(() => {
                    showLoading(false);
                });
        ...
        function renderizarDetalhesVenda(venda) {
            document.getElementById('detalhesVendaNumero').textContent = '#' + venda.numero;
            document.getElementById('detalhesData').textContent = venda.data;
            document.getElementById('detalhesCliente').textContent = venda.cliente;
            document.getElementById('detalhesPagamento').textContent = venda.forma_pagamento;
            document.getElementById('detalhesTotal').textContent = Number(venda.valor_total).toLocaleString('pt-AO') + ' Kz';
            document.getElementById('detalhesEntregue').textContent = Number(venda.valor_entregue).toLocaleString('pt-AO') + ' Kz';
            document.getElementById('detalhesTroco').textContent = Number(venda.troco).toLocaleString('pt-AO') + ' Kz';

            const itensBody = document.getElementById('detalhesItensBody');
            if (venda.itens && venda.itens.length > 0) {
                itensBody.innerHTML = venda.itens.map(item => `
                    <tr>
                        <td style="padding:8px 16px;">${item.produto_nome}</td>
                        <td style="padding:8px 16px;text-align:center;">${item.quantidade}</td>
                        <td style="padding:8px 16px;text-align:right;">${Number(item.preco_unitario).toLocaleString('pt-AO')} Kz</td>
                        <td style="padding:8px 16px;text-align:right;">${Number(item.subtotal).toLocaleString('pt-AO')} Kz</td>
                    </tr>
                `).join('');
            } else {
                itensBody.innerHTML = `
                    <tr>
                        <td colspan="4" style="text-align:center;padding:16px;color:var(--text-light);">Nenhum item encontrado</td>
                    </tr>
                `;
            }

            const obsContainer = document.getElementById('detalhesObservacoes');
            const obsText = document.getElementById('detalhesObsTexto');
            if (venda.observacoes) {
                obsContainer.style.display = 'block';
                obsText.textContent = venda.observacoes;
            } else {
                obsContainer.style.display = 'none';
            }
        }

        function carregarEImprimirVenda(vendaId) {
            showLoading(true);
            ...
            fetch(`/cooperativas/${COOPERATIVA_ID}/vendas/${vendaId}`)
                ...
                .then(data => {
                    if (data.success) {
                        imprimirFatura(data.venda);
                    } else {
                        showToast('Erro', data.message || 'Falha ao carregar dados da venda.', 'danger');
                    }
                })
                .catch(err => {
                    console.error('Erro ao carregar venda para impressão:', err);
        ...
        /* =============================================
           EVENTOS - VER E IMPRIMIR
        ============================================= */
        document.addEventListener('click', function (e) {
            const btnVer = e.target.closest('.btn-ver-venda');
            if (btnVer) {
                const vendaId = btnVer.dataset.id;
                carregarDetalhesVenda(vendaId);
                return;
            }

            const btnImprimir = e.target.closest('.btn-imprimir-venda');
            if (btnImprimir) {
                const vendaId = btnImprimir.dataset.id;
                carregarEImprimirVenda(vendaId);
                return;
            }
        });
